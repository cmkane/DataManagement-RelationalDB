...
RC privateTestCase_7() {
    ...
    *(float *) value2.data = 35.0; // C[10.0-35.0], then B: a ... zzzzzzzzzzzzzzzzzzzzzzzzzz
    ...
    // Create GHJoin
    auto *ghJoin = new BNLJoin(filter, rightIn, cond, 5);
    ...
    while (ghJoin->getNextTuple(data) != QE_EOF) {
        ...
        // Print right.C
        std::cerr << "rightvarchar.B " <<
        std::cerr << "  C " << *(float *) ((char *) data + offset + 1) << std::endl;
    ...
    if (expectedResultCnt != actualResultCnt) {
        std::cout << "expected = " << expectedResultCnt << std::endl;
