...
int RBFTest_10(RecordBasedFileManager &rbfm) {
    ...
    for (int i = 1; i < numRecords; i++) {
        ...
        if (memcmp(returnedData, record, sizes[i]) != 0) {
            cout << "[FAIL] The size of the data is: " << sizes[i] << endl;
            cout << "[FAIL] The record to read is on page: "<< rids[i].pageNum << " and slot: " << rids[i].slotNum << endl;
            cout << "[FAIL] Test Case 10 Failed!" << endl << endl;
            cout << "[FAIL] Test Case 10 failed on iteration: " << i << endl;
            cout << "[FAIL] The original data is: " << endl;
            rbfm.printRecord(recordDescriptor,record);
            cout << "[FAIL] The read data is: " << endl;
            rbfm.printRecord(recordDescriptor,returnedData);
            free(nullsIndicator);
            free(record);
            free(returnedData);
